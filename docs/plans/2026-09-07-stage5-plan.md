...
section before any task). Design §3 and the Amendments list the measured facts;
`docs/plans/2026-09-07-stage5-gate5-handover.md` carries the session state and expected numbers
(the pre-gate-4 handover is retired into the addendum at the end of this file).
...
- [x] **Step 1: Add the staging tree to `.gitignore`**
...
- [x] **Step 2: Declare the already-installed transitive dependencies as direct ones**
...
Measured 2026-09-07: `uv add --offline` (all three cached); `uv.lock` +6 lines, dependency and
`requires-dist` metadata only.

- [x] **Step 3: Register the markers**
...
- [x] **Step 4: Write the failing test for the helper**
...
Measured 2026-09-07: `ImportError: cannot import name 'work_chapter'` at collection, as expected.

- [x] **Step 6: Implement the helper**
...
- [x] **Step 7: Run the whole unit tier**

Run: `uv run pytest -q && uv run ruff check . && uv run ruff format --check .`
Expected: 105 passed (103 + 2), ruff clean.
Measured 2026-09-07: 105 passed, 4 deselected in 0.36 s; `ruff check` all passed; 40 files already
formatted.

Addendum 2026-09-07 (gate 4): `tests.helpers.work_chapter_sandbox(book, chapter, root, monkeypatch)`
— a `workdir` test that runs the pre-pass or the checks does so on a copy (stage-2 outputs and
`chapter.pdf` symlinked, the stage 3–5 JSON copied, `src.config.WORK` redirected). A plain
`uv run pytest` had rewritten the real bma ch05 `qa_report.json` and invalidated a certified
render's `qa_report_sha256`. The two writing tests in `tests/test_prepass.py` use it; the read-only
`workdir` tests keep `work_chapter`.
...
Measured 2026-09-07: both modules `ModuleNotFoundError` at collection (`src.fold`, `src.textlayer`).

- [x] **Step 3: Implement `src/fold.py`**
...
- [x] **Step 4: Implement `src/textlayer.py`**
...
Measured 2026-09-07: 12 passed in 0.53 s (4 fold, 8 textlayer; both `workdir` tests ran). Unit tier
...
Measured 2026-09-07: `ModuleNotFoundError: No module named 'src.patches'` at collection.

- [x] **Step 3: Implement `src/patches.py`**
...
Run: `uv run pytest tests/test_patches.py -v`
Expected: all PASS.
Measured 2026-09-07: 9 passed in 0.02 s. Unit tier 126 passed, 4 deselected; ruff clean.

Addendum 2026-09-07 (after the Task 11 dry run): a sixth op, `set_inline_math` (reviewer-only;
`old` = the exact LaTeX of exactly one `equation_inline` span of a text-bearing or captioned block,
`new` non-empty), because `replace` reaches only `text` spans and bma ch05 hunks 2 and 29 are MFR
inline-formula defects (`$` read as `\\mathbb{S}`; a footnote marker glued to `\\ldots`). One test
added; the `review patch` CLI takes its choices from `OPS`. Design §6/§11 amendment at Task 14.

Addendum 2026-09-07 (gate 4): while the operator's reviewer session and the implementation session
both wrote to bma ch05, the session's `make review` (skill step 1) reran the pre-pass, which rebuilt
every hunk entry and wiped the 13 manual verdicts (the `rv-` patches survived; block, spot,
heading and footnote verdicts were untouched). `prepass.run` now keeps entries whose `rule` is
`manual` and counts them as `manual`; test added. The lost verdicts were reconstructed from the
pre-wipe histogram (manual 13 = patched 3 with `rv-0005/6/7`, move 7, unresolved 3) and re-entered.
Rule for the skill and the operator: one writer per work dir at a time.
...
- [x] **Step 2: Add `make_work_dir` to `tests/helpers.py`**
...
Measured 2026-09-07: `ImportError: cannot import name 'validate_review' from 'src.qa_schema'`.

- [x] **Step 4: Append `validate_review` to `src/qa_schema.py`**
...
- [x] **Step 5: Implement `src/review.py`**
...
Measured 2026-09-07: 12 passed in 1.02 s. `hybrid_auto_dir(wd)` does resolve to `wd/chapter/hybrid_auto`;
`make_work_dir` calls it anyway. `ruff check` raised PLW1510 on the plan's `_cli` helper
(`subprocess.run` without `check`); `check=False` added to the test and to the block above. Unit
tier 138 passed, 4 deselected.

- [x] **Step 7: Commit**

```bash
uv run pytest -q && uv run ruff check . && uv run ruff format --check .
git add src/config.py src/qa_schema.py src/review.py tests/helpers.py tests/test_review.py
...
Measured 2026-09-07: `ModuleNotFoundError: No module named 'src.prepass'` at collection.

- [x] **Step 3: Implement `src/prepass.py`**
...
Measured 2026-09-07: first run 13 of 15 passed; both real-chapter failures were data, not bounds.
(a) bma ch05 `p009-b019` was not dropped: the guardrail merged the two page-9 `#` icons into one
hunk (md core `# #`, anchor `p009-b015`), so the junk rule dropped only the anchor; `p012-b023`, a
third `#` icon with its own hunk, was dropped and is missing from the handover's list. Fix: a junk
verdict also drops the following text-bearing blocks without text-layer words whose tokens consume
the rest of the core (`Verdict.also`, one `drop_block` patch each; the core must be fully
consumed, else `needs_eyes`). Two such hunks in the fleet: bma ch05 hunk 19, corpfin ch05 hunk 36.
(b) bma ch06 `p026-b007` has no whole-block period: its 1560 tokens are seven line segments (17,
16, 16, 17, 14, 18, 6 tokens) each emitted 15 times. Fix: `duplicate_unit` collapses every run of
adjacently repeated segments (the repeat consuming the most tokens wins at each position; 0.00 s),
and `classify` accepts the collapse only when the bbox text-layer tokens equal the collapsed tokens
(104 = 104 here; page-level `find_run` also confirms but takes 1.9 s). An unconfirmed proposal
falls through to the later rules, since the collapser also proposes `Page 83 Page 83` → `Page 83`
and the folio rule owns that; the note is kept when the hunk ends unresolved. Three unit tests
added (line repeats with different periods; text-layer confirmation; a junk hunk spanning two
blocks). After the fixes: 18 passed in 1.75 s; bma ch05 57 hunks, `needs_eyes` 13; unit tier 156
passed. `ruff check` raised SIM905 ×8 on the plan's `"…".split()` literals (auto-fixed, mirrored
above).

- [x] **Step 5: Measure all eight chapters and paste the numbers**
...
Measured 2026-09-07 (622 hunks = guardrail total):

| chapter | hunks | same_letters | inline_math | move | edge_glyph | junk | duplicate | folio | needs_eyes |
|---|---|---|---|---|---|---|---|---|---|
| acct ch05 | 38 | 6 | 0 | 12 | 1 | 8 | 0 | 0 | 11 |
| bkm ch05 | 111 | 12 | 47 | 12 | 11 | 0 | 0 | 0 | 29 |
| bma ch05 | 57 | 26 | 4 | 8 | 4 | 2 | 0 | 0 | 13 |
| bma ch06 | 82 | 46 | 3 | 10 | 6 | 4 | 5 | 0 | 8 |
| corpfin ch05 | 40 | 18 | 3 | 8 | 3 | 1 | 0 | 0 | 7 |
| ops ch05 | 81 | 2 | 24 | 17 | 1 | 0 | 0 | 7 | 30 |
| stats ch05 | 140 | 7 | 84 | 6 | 0 | 0 | 0 | 0 | 43 |
| strat ch05 | 73 | 42 | 0 | 11 | 8 | 0 | 0 | 0 | 12 |
| total | 622 | 159 | 165 | 84 | 34 | 15 | 5 | 7 | 153 |

Against the expected shape: `same_letters` leads in bma ch05, bma ch06, corpfin and strat;
`inline_math` leads in bkm and stats because it is tested first (handover §4 anticipated this);
`move` leads in acct; `needs_eyes` leads in ops (30). `junk`: bma 6 (2 + 4), corpfin 1, acct 8 —
not ≥ 11: acct's other two insertion residues are paragraphs with 39 and 14 text-layer words.
`duplicate`: bma ch06 only (five hunks, one block, one patch). `folio`: ops only (7). The 153
`needs_eyes` by note: inline math differs 63, one-sided core 44 (4 after a move strip), anchored to
a table/chart/image (masking gap) 20, letters differ 10, raw run or md window not unique 15
(same_letters 12, edge_glyph 3), anchor block missing 1 (ops). No mechanical class is left in
the residue; it is the reviewer's work order.
...
Measured 2026-09-07: `ModuleNotFoundError: No module named 'src.sections'` at collection.

- [x] **Step 3: Implement `src/sections.py`**
...
Measured 2026-09-07: 15 passed in 0.21 s on the first run. Real chapters: bma heading 9; acct
heading 9 + first_child 1; ops heading 9 + chapter_start 1; corpfin heading 7; stats heading 7;
bkm and strat raise `SectionError` naming "End of Chapter Material" and "Experiential Exercise"
(the rename branch waits for Task 13). `ruff check` raised I001 on the plan's two `src.blocks`
import lines; merged by `--fix` and mirrored above.
...
Measured 2026-09-07: `ModuleNotFoundError: No module named 'src.render'` at collection.

- [x] **Step 3: Implement `src/render.py`**
...
Run: `uv run pytest tests/test_render.py -v`
Expected: all PASS.
Measured 2026-09-07: 11 passed in 0.04 s on the first run; ruff clean without fixes. Unit tier 182
passed, 4 deselected.
...
Measured 2026-09-07: `ModuleNotFoundError: No module named 'src.review_checks'` at collection.

- [x] **Step 3: Add the per-book review flag to `config/books.yaml`**
...
- [x] **Step 4: Implement `src/review_checks.py`**
...
Measured 2026-09-07: first run 5 of 8 passed; the synthetic-page test passed as written. Three
fixes, all in the module (the tests stand): (a) `latex_ok` counted `\left`/`\right` occurrences, so
`\left( x \right` (a `\right` without a delimiter) passed — the pairing now counts *delimited*
`\left`/`\right` (`\langle`, `\{`, `(`, `.`, `|`…, not `\leftarrow`) and requires the delimited
count to equal the raw count; the unescaped-`$` test runs before pylatexenc, which otherwise
reports an unclosed math mode for `a $ b`; `\begin{`/`\end{` counted with the brace. (b) The
table inline-math regex `\$([^$]+)\$` spanned from the literal `$` in `($)` to the next `$`; math
content must now start and end with a non-space character. (c) `bag` folds after
`punctuation_variants`, so U+2212 and the dashes equal `-` as in the guardrail rules. (d) The
missing-anchor test did not raise: a first section on page 0 without a heading fell into
`chapter_start` even with no chapter title on the page. `_after_chapter_title` now requires a
level-1 title first (design §9 rule 3 reads "after the chapter title"); ops, bma and corpfin
carry one at `p000-b000`, so the real chapters are unchanged. RED test added in
`tests/test_sections.py`. After the fixes: 8 passed (+16 sections); ruff clean.

- [x] **Step 6: Run the checks on bma ch05 and paste the summary**
...
```text
review-checks: bma ch05 {'formulas': 20, 'formulas_textlayer': 19, 'formulas_parse': 18, 'tables': 38,
  'tables_textlayer': 32, 'tables_rectangular': 38, 'tables_empty': 0, 'subheadings_missing': 0,
  'footnotes_unmatched': 0}
review-checks: stats ch05 {'tables': 40, 'tables_textlayer': 25, 'tables_rectangular': 40, 'tables_empty': 0,
  'formulas': 78, 'formulas_textlayer': 0, 'formulas_parse': 78, 'subheadings_missing': 2,
  'footnotes_unmatched': 1}
review-checks: acct ch05 {'tables': 80, 'tables_textlayer': 45, 'tables_rectangular': 65, 'tables_empty': 15,
  'formulas': 1, 'formulas_textlayer': 1, 'formulas_parse': 1, 'subheadings_missing': 0,
  'footnotes_unmatched': 0}
```

bma tables 32 of 38 bag-equal after `$…$` flattening (9 before); two bma formulas fail the strict
parse (tier B will see them); stats subheadings 12 of 14 and one unreferenced footnote definition,
as the handover measured; acct's 65 rectangular tables are exactly its 65 non-empty ones, and its
heading runs now find all subheadings (8 of 10 with the old single-block fold). Unit tier 191
passed, 4 deselected.
...
Measured 2026-09-07: all three modules `ModuleNotFoundError` at collection (`vault_commit`,
`vault_checks`; `finalize` missing from `src.review`). `check=False` added to the two
`subprocess.run` calls above (PLW1510).

- [x] **Step 3: Implement `src/vault_commit.py` (render and check)**
...
- [x] **Step 4: Implement `src/vault_checks.py`**
...
- [x] **Step 5: Add `unresolved_items`, `finalize` and the subcommand to `src/review.py`**
...
Run: `uv run pytest tests/test_vault_commit.py tests/test_vault_checks.py tests/test_finalize.py -v`
Expected: all PASS.
Measured 2026-09-07: 19 passed in 0.56 s on the first run; unit tier 210.

- [x] **Step 7: Render bma ch05 for inspection and run the battery**
...
Measured 2026-09-07: 10 notes + 10 assets as expected —

```text
vault/bma/ch05/: assets manifest.json
  bma-ch05-00-net-present-value-and-other-investment-criteria.md
  bma-ch05-01-a-review-of-the-net-present-value-rule.md
  bma-ch05-02-the-payback-and-accounting-rate-of-return-rules.md
  bma-ch05-03-the-internal-rate-of-return-rule.md
  bma-ch05-04-choosing-capital-investments-when-resources-are-limited.md
  bma-ch05-05-key-takeaways.md  bma-ch05-06-further-reading.md  bma-ch05-07-problem-sets.md
  bma-ch05-08-solutions-to-self-test-questions.md  bma-ch05-09-mini-case-vegetrons-cfo-calls-again.md
vault/bma/ch05/assets/: bma-ch05-p002-b000.jpg p003-b000 p008-b009 p011-b000 p011-b002 p011-b006
  p013-b007 p016-b004 p026-b005 p026-b007 (.jpg)
```

The battery was **not** OK on the first render: 95 problems, 92 of them `$` accounting. bma ch05's
text spans carry 354 literal `$` (currency; the fleet's table HTML has 317 `$`+digit) and neither
design §9 nor the battery had a rule for a currency sign. Rendering rules added, each RED first:
(a) `md_text` escapes every `$` in text spans and captions as `\$` — a text span never holds math,
which lives in `equation_inline` spans; (b) pipe-table cells keep `$…$` pairs that read as LaTeX
(letter-, `\`-, `(`- or `{`-led, or digit-led containing `\ ^ _ =` such as `$1,949^a$`) and
escape the other `$` (`\$4,000`, `(\$)`, `\$ 5,000-\$6,000`), rescanning from the next `$` when
a pair was accidental; (c) `aside_callout` quotes every line of a multi-line aside block (one
unquoted continuation in section 03); (d) `inline` restores the space mineru drops between an
inline formula and a following alphanumeric text span (`$C_0$million` → `$C_0$ million`; 164 such
joins across six chapters: bma 19, bkm 35, ops 33, stats 75, corpfin 2). Battery fixes: only
unescaped `$` count; `<table>` HTML is excluded from the inline scan (Obsidian renders HTML blocks
as HTML, so a `$` there is literal text — and `$C_0$` inside a `<td>` shows literally, a fidelity
limit for the gate-4 mobile pass); inline pairs are taken in order of the paragraph's unescaped
`$` positions — the `_BAD_INLINE` regex matched the text *between* two formulas (12 false
positives). Result: `check: vault/bma/ch05 2 problem(s)` — both `display math does not parse` in
the mini-case note, where the MFR wrote `\\$ 110,000` (double-escaped dollar); `latex_parses:
false` already hands them to the reviewer, so an unreviewed chapter is expected to show them.
Reading pass on notes 01 and 03: headings, page comments, display math with tags, escaped currency
and the complex-table HTML read right; `<sup>⟩</sup>` is how mineru typed the `◗` caption
ornament (not stripped, cosmetic, left for the stop rule). Unit tier 216 passed, 4 deselected;
ruff clean.
...
Measured 2026-09-07: `ImportError: cannot import name 'push_chapter'`; the live test errored at
collection until Step 4 defined its import, then was deselected (5 deselected in the tier).

- [x] **Step 3: Add the client and push to `src/vault_commit.py`**
...
- [x] **Step 4: Add the rendered check to `src/vault_checks.py`**
...
Run: `uv run pytest tests/test_vault_push.py -v`
Expected: all PASS.
Measured 2026-09-07: 3 passed in 0.12 s; I001 on the live test's imports fixed and mirrored
above; unit tier 219.

- [x] **Step 6: Live probe — operator has Obsidian open on 192.168.3.26**
...
Measured 2026-09-07 (Obsidian 1.13.7, plugin 4.1.3, `authenticated: true`): the round trip passed
(PUT 204 for the note and the PNG, both read back byte-equal) — but **the REST API does not render**.
`GET /vault/<path>` with `Accept: text/html` answered `text/markdown` (the raw note), and the
plugin's own `/openapi.yaml` lists only `text/markdown`, `application/json`,
`application/vnd.olrapi.note+json` and `application/vnd.olrapi.document-map+json` for that GET; no
path in the spec renders HTML. There are no class names to record; `rendered_counts`,
`source_counts`, `check_rendered` and the `RENDERED_*` regexes were removed. The first pytest run
failed on the (unattainable) math count before its DELETEs, leaving the two probe files in the
vault; they were deleted by hand and the live test now cleans up in `finally`.

What Obsidian *does* expose is its own parse, probed on a synthetic note carrying frontmatter,
three heading levels, inline and display math, a pipe and an HTML table, a resolved and an
unresolved embed, a resolved and an unresolved link, a callout and a footnote:

```text
note+json      frontmatter: identical to PyYAML's (section '5.10' stays a string, null stays null)
               content: byte-equal to the PUT body
               links: ONLY the resolved targets — ["…/probe.md", "…/probe.png"]; the unresolved
                      ![[nope.png]] and [[nope-note]] are absent
document-map   headings: ["Title", "Title::Sub", "Title::Sub::Deeper"]; blocks: []; frontmatterFields
```

So `check --rendered` became `check --parsed` (`vault_checks.check_parsed`, `note_json` with a
short retry for the metadata cache): per pushed note, Obsidian's frontmatter must equal ours, its
content must equal the staging file, every `[[…]]` and `![[…]]` target must appear among the
resolved `links`, and the document map's heading count must equal the source's. Math and table
rendering cannot be verified over REST; that is what the gate-4 mobile pass (Task 12) confirms —
design decision 6 and §10 to be amended at Task 14. Three fake-client tests cover `check_parsed`
(clean after a push; unresolved embed + content drift + heading drift; not pushed). The rewritten
live test asserts the parse facts above and passed (1 passed in 0.14 s); the vault root and
`raw/` were listed afterwards and carry no probe folder. Unit tier 222 passed, 5 deselected;
ruff clean.

Addendum 2026-09-07 (first real `check --parsed`, bma ch05): the document map lists heading *paths*,
unique and `::`-joined, and the parent of a heading is the nearest preceding heading of a smaller
level — two sibling `### BEYOND THE PAGE` collapse to one path (note 03), and a `###` directly
under the `#` is its child (notes 04, 07). A heading count was therefore the wrong comparison;
`heading_paths` builds the same set from the source and the sets must be equal.
...
- Measured 2026-09-07: the Makefile targets were pulled forward (Task 14 Step 1), so `SKILL.md`
  is written against `make review` / `make finalize` for the composite steps and
  `uv run python -m src.review …` for every write; it also states that the tier-A agents write
  nothing, that a refused patch writes nothing, and that make exits 2 on `needs_attention` while
  the printed status line is the signal. Rubrics and the schema are as planned. Step 4 (the
  operator's dry run) is open.
...
- [x] **Step 2: Write the four rubrics**
...
- [x] **Step 3: Write `answer-schema.json`**
...
- [x] **Step 4: Dry run on bma ch05 in one Claude Code session**
...
Measured 2026-09-07 (operator's session, 05:44–06:22 UTC): `prepass.counts` same_letters 26,
needs_eyes 13, move 8, edge_glyph 4, inline_math 4, junk 2. Tier A: 67 of 67 crops answered,
65 `match: true`, 2 mismatches (both display formulas with the MFR's double-escaped dollar), 16
spot checks `ok`. Tier B patches 13: `set_math` 2, `set_content` 6, `replace` 5; hunk verdicts
accepted_pdf 30, move 15, verified 4, patched 3, dropped 2, unresolved 3; blocks verified 60,
patched 7; headings 9/9 and 12/12; footnotes 11 defs / 12 refs, ref 16 dangling;
`crops_opened` 87; battery clean. `finalize → needs_attention` with 3 unresolved hunks. Checked
against the data: hunk 6 ("footnote 1 dropped") is not a defect — the footnote is `p004-b017`
and the hunk is a guardrail alignment artefact (verdict `move`); hunks 2 and 29 are genuine MFR
inline-formula defects that no patch op could reach → `set_inline_math` added (Task 3 addendum).
The three were closed from the implementation session at the operator's request (hunk 6 `move`;
hunks 2 and 29 `patched` with `rv-0014`, `rv-0015`+`rv-0016`); after the prepass-rerun incident
(Task 3 addendum) the 11 wiped manual verdicts were re-entered from the pre-wipe histogram, and
`make finalize BOOK=bma CH=5` printed `certified`: hunks move 16, patched 5, accepted_pdf 30,
verified 4, dropped 2 (13 manual); blocks verified 60, patched 7; spot checks 16 ok; 16 `rv-`
patches; battery clean; manifest `certified`, 20 files; notes `certified: true`,
`certified_at: '2026-09-07'`. `/cost` is not available under the operator's subscription; no
token figure for this session.
...
- [x] **Step 1: Certify both chapters**
...
Measured 2026-09-07: both `certified`. bma ch06 (operator's session, 06:54–07:08 UTC): pre-pass
same_letters 46, move 10, needs_eyes 8, edge_glyph 6, duplicate 5, junk 4, inline_math 3; hunks
accepted_pdf 52, patched 11, move 10, dropped 5, verified 3, accepted_md 1 (8 manual); tier A 50 of
50 answered — table 30/31 match, formula 12/18, chart 0/1, the 8 mismatches patched; blocks
verified 43, patched 7; spot checks 19 ok + 1 patched; headings 10/10 and 22/22; footnotes 13
definitions / 13 references, none dangling; 17 `rv-` patches (`set_math` 6, `replace` 5,
`set_inline_math` 4, `set_content` 1, `drop_block` 1); `crops_opened` 81; battery OK; manifest
`certified`, 12 files. bma ch05: Task 11 Step 4.

- [x] **Step 2: Push and run the rendered check**
...
Measured 2026-09-07, bma ch05 (`--rendered` is `PARSED=1`, Task 10): the first `make push` refused
with a stale `qa_report_sha256` — a pytest run had rerun the pre-pass on the real work dir after
finalize (verdicts intact thanks to the Task 3 addendum; fixed by the sandbox helper, Task 1
addendum). `make finalize` → `certified` again; `make push` → 20 files to
`raw/textbooks/bma/ch05/`, 0 failed read-back, 20 verified; `make check PARSED=1` → OK after the
heading-path fix (Task 10 addendum). Cosmetic, left for the stop rule: mineru typed the Connect
logo text and one sentence of note 07 as `title` blocks, so they render as `###` headings.
bma ch06 (07:23 UTC): `make push` → 12 files to `raw/textbooks/bma/ch06/`, 0 failed read-back, 12
verified; `make check PARSED=1` → OK on the first run.

- [x] **Step 3: Mobile pass (operator)**
...
Measured 2026-09-07, bma ch05 (operator's Android phone, Obsidian Sync delivered the push within
minutes): 7 of 7 — (1) inline and display math in 5-1 and equation 5.1; (2) a pipe table and an HTML
table in 5-2; (3) the Figure 5.1 flowchart crop and the Figure 5.2 bar chart in 5-1; (4) the Chart
data and BEYOND THE PAGE callouts fold; (5) the `[^1]` in 5-2's first paragraph jumps to
"Occasionally firms…" and returns; (6) ← · ↑ · → from the last line of 5-3 open 5-2, the hub and
5-4; (7) `%% p. 119 %%` and its siblings are invisible in reading view and present in source view.
ch06 (07:2x UTC, sections 6-1, 6-3, 6-4): footnote jump/return, ← · ↑ · → and invisible page
comments all pass; the chapter's one figure (`p003-b000`) renders in 6-1.
...
Measured 2026-09-07: **gate 4 passed** — `metrics/gate4-bma.md` (both certified, 20/20 and 12/12
pushed and verified, `check PARSED=1` OK twice, checklist 7/7 on both chapters). Two fixes went back
into the code on the way (Task 3 addendum: `set_inline_math`, pre-pass keeps manual verdicts; Task 1
addendum: sandboxed `workdir` tests).
...
Measured 2026-09-07: `ModuleNotFoundError: No module named 'scripts.benchmark_report'` at collection.
The test imports `scripts.benchmark_report` the way `tests/test_patch_toc.py` imports
`scripts.patch_toc` (pytest `pythonpath = ["."]`), not through `sys.path`. Two tests added to the
plan's: `reviewed_dirs` needs the `review` key and honours a `--chapters` filter (the handover's
extra work dirs bkm ch15, ch20–23 and bma ch09 would otherwise enter the gate-5 table), and a
read-only `workdir` test on certified bma ch05 against the gate-4 record (67 flagged, tier A 65/2,
16 review patches, 87 crops, `effort` span 70 min).

- [x] **Step 2: Implement `scripts/benchmark_report.py`**
...
Measured 2026-09-07: as above plus `--chapters BOOK/chNN …` (`reviewed_dirs(work, only)`), an
`edge_glyph_sup` column — design §15's "how many were footnote markers": prepass `edge_glyph`
patches whose `new` carries `<sup>` — and the per-book notes prefilled with the review patches by
op (`review_ops`). Project ruff rules: EXE001 (shebang removed), RUF100 (`noqa: E402` unused),
DTZ011 (`datetime.now(timezone.utc).date()`).
...
Measured 2026-09-07: 3 passed in 0.12 s; unit tier 229 passed, 5 deselected; ruff clean.

- [x] **Step 4: Outline renames for bkm and strat, re-split, re-check the anchors**
  ...
  bkm:
    toc:
      ...
      patches:
        ...
        - {rule: rename, level: 3, match: '^End of Chapter Material$', replace: 'Summary'}
        # 15 titles carry an unprinted U+0007 before the words; MuPDF re-encodes it as U+00B7 on
        # any outline save, so it is removed at the source (2026-09-07, gate 5).
        - {rule: rename, level: 3, match: '\x07', replace: ''}
        - {rule: rename, level: 5, match: '\x07', replace: ''}
```
...
Measured 2026-09-07: RED first — bkm `{"heading": 9}` and strat `{"heading": 24}` rows added to
`test_real_chapters_resolve` failed with `SectionError` naming the two titles. Dry runs: bkm 28 of
832 entries (the label recurs in every chapter), strat 2 of 442 (ten chapters were already
plural). strat `--apply` clean. bkm `--apply` wrote all 28 renames but raised `outline after save
differs from the patched list`: MuPDF re-encodes the U+0007 control character that 15 *other* bkm
titles carry (13 at level 3 after `N.N: `, 2 at level 5) as U+00B7 `·` at `set_toc` time, so the
strict post-save comparison failed on entries the rule never touched — and the `·` passed
`clean_title` (which strips only `Cc`) into `meta.json` (`·Measuring Returns …`). Two fixes, RED
first: (a) `scripts/patch_toc.py` takes its reference outline from `doc.get_toc()` after `set_toc`
(`_mupdf_encoding`), reports such entries as `Change(…, "control character re-encoded by MuPDF")`
in dry run and apply alike, and still raises on any other difference; two tests build the fixture
by writing `/Title` as UTF-16BE on the outline item's xref (`set_toc` cannot write a raw U+0007).
(b) The two `'\x07'` rename rules above remove the character at the source (the block mirrors
`config/books.yaml`). `bkm.pdf` restored from `bkm.orig.pdf`; dry run and `--apply` with the
three rules: 43 of 832 renamed, 0 re-encoded, exit 0. `make split`: bkm 9 sections (first
`Measuring Returns over Different Holding Periods`, last `Summary`), strat 24. Both outlines
afterwards: 0 control characters, 0 `·`, 0 old labels; 1073 / 509 pages unchanged;
`check_books.py` 7 OK → `books/manifest.json` carries the new sha256 of bkm and strat (outline
sizes unchanged). `patches.json` stays bound (bkm 22, strat 48 patches; content lists untouched).
The renames test had a bug of its own — `"Experiential Exercise"` is a prefix of the plural — and
now names the full old title; it passes through its `else` branch. `tests/test_sections.py -m
workdir` 8 passed; unit tier 233 passed, 5 deselected; ruff clean. Not re-split (outside the
gate): bkm ch15 and ch20–23 keep the old label and the U+0007 in their `meta.json` until
`make split`.
...
# check accepts PARSED=1 (Obsidian's parse of the pushed notes; the API renders no HTML).
SECTION ?=
ROOT ?= raw/textbooks/
PARSED ?=
...
Add the eight targets to `.PHONY` and one `@echo` line each to `help`, in the style of the
existing stage lines.

Measured 2026-09-07 (pulled forward before Task 11 at the operator's request, so the skill lands in
its `make review` form): all targets except `benchmark-report` added, plus `finalize` (the skill's
last step) and `PARSED=1` in place of `RENDERED=1` (Task 10). GNU make exits 2 on any failing
recipe, so `make finalize` callers read the printed status; the 0/3/1 codes belong to
`uv run python -m src.review finalize`. `benchmark-report` waits for Task 13's script.
...
Measured 2026-09-07: `make review` (57 hunks; checks summary unchanged), `make render … SECTION=1`
(20 files) and `make check` ran; check reported the two known MFR display formulas of bma ch05
(exit 2 through make). `make help` grep = 6 (7 once `benchmark-report` exists). `make -n finalize`
shows the `src.review finalize` command; `make check BOOK=x` fails with the usage line.
...
Measured 2026-09-07: the Commands paragraph and two bullets (`make review` → `/chapter-review` →
`make finalize`; `make render|check|push`, `PARSED=1`, `OBSIDIAN_*`, `ROOT`) were synced when the
targets landed (the sync rule fires with the Makefile). Layout and Hard rules remain for Task 14.
...
## Addendum — 2026-09-07 (gate 4 closed): handover retired

Gate 4 passed on bma ch05/ch06 (`10430e4`; record in `metrics/gate4-bma.md`). The session-bridging
file `docs/plans/2026-09-07-stage5-handover.md` (state, reading order, protocol, expected numbers) is
retired with this addendum; `docs/plans/2026-09-07-stage5-gate5-handover.md` takes over for Task 13
and Task 14. Its still-live content moved there: the eight-chapter expected numbers (§4), the
engineering notes (§6, extended with the cwd, heredoc and one-writer lessons), and the design
amendments due at Task 14 (§5). Nothing else in the retired file is current: its §2 state and §3
protocol are superseded by the "Measured 2026-09-07" notes on Tasks 1–12 above.
