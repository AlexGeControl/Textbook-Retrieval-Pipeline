...
def blk(text, i, page=0, type="text"):
    return Block(
        id=block_id(page, i), index=i, type=type, page_idx=page, bbox=(0, 0, 1, 1), text=text
    )


def test_discarded_masks_take_header_and_page_number_only(table_cache):
    middle = json.loads((table_cache / "table_middle.json").read_text())["pdf_info"]
    masks = discarded_masks(middle[0], pad=0)
    assert len(masks) == 2  # header + page_number; the page_footnote stays
    assert any(abs(m.y0 - 34) < 1 and abs(m.x0 - 184) < 1 for m in masks)


def test_block_masks_scale_content_list_bboxes_to_points(table_cache):
    blocks = load_blocks(table_cache / "table_content_list.json")
    table = next(b for b in on_page(blocks, 0) if b.type == "table")
    (rect,) = block_masks([table], pymupdf.Rect(0, 0, 576, 720), pad=0)
    assert (
        abs(rect.x0 - table.bbox[0] * 0.576) < 0.01 and abs(rect.y1 - table.bbox[3] * 0.72) < 0.01
    )
    text_only = [b for b in on_page(blocks, 0) if b.type == "text"]
    assert block_masks(text_only, pymupdf.Rect(0, 0, 576, 720)) == []


def test_pdf_page_text_masks_running_header_and_table_cells(table_cache):
    middle = json.loads((table_cache / "table_middle.json").read_text())["pdf_info"]
    blocks = load_blocks(table_cache / "table_content_list.json")
    page = pymupdf.open(FIXTURES / "bma" / "table.pdf")[0]
    masks = discarded_masks(middle[0]) + block_masks(on_page(blocks, 0), page.rect)
    text = pdf_page_text(page, masks)
    assert "Suppose you were given" in text
    assert "Chapter 6" not in text and "Making Investment Decisions" not in text
    assert "Less depreciation" not in text  # a table cell
    assert "\n" in text  # line structure preserved for dehyphenation


def test_align_matches_blocks_out_of_order():
    a = align_blocks(
        pdf_tokens("a b c g h i d e f"), [blk("a b c", 0), blk("d e f", 1), blk("g h i", 2)]
    )
    assert (a.residual_pdf, a.residual_md) == ([], [])
    assert a.spans == {"p000-b000": (0, 3), "p000-b001": (6, 9), "p000-b002": (3, 6)}


def test_align_prefers_the_next_occurrence_in_reading_order():
    a = align_blocks(pdf_tokens("x y x y"), [blk("x y", 0), blk("x y", 1)])
    assert a.spans == {"p000-b000": (0, 2), "p000-b001": (2, 4)}


def test_align_leaves_unmatched_material_in_the_residue():
    a = align_blocks(pdf_tokens("a b c d e f"), [blk("a b c", 0), blk("d X f", 1)])
    assert [t.text for t in a.residual_pdf] == ["d", "e", "f"]
    assert [(t.text, t.block) for t in a.residual_md] == [
        ("d", "p000-b001"),
        ("X", "p000-b001"),
        ("f", "p000-b001"),
    ]
...
def test_align_skips_table_bodies_but_aligns_their_captions():
    table = Block(
        id="p000-b000",
        index=0,
        type="table",
        ...
        bbox=(0, 0, 1, 1),
        table_body="<td>a</td>",
        captions=("Table 1 Cash",),
    )
    a = align_blocks(pdf_tokens("Table 1 Cash a b"), [table, blk("a b", 1)])
    assert (
        a.spans == {"p000-b000": (0, 3), "p000-b001": (3, 5)}
        and a.residual_pdf == []
        and a.residual_md == []
    )
    bare = Block(
    ...
    assert align_blocks(pdf_tokens("a b"), [bare, blk("a b", 3)]).spans == {"p000-b003": (0, 2)}


def test_diff_identical_streams_yield_no_hunks():
    hunks, _ = diff_page(pdf_tokens("a b c d"), [blk("a b c d", 0)], 119, 0)
    assert hunks == []


def test_diff_replace_hunk_has_schema_keys_and_context():
    hunks, _ = diff_page(pdf_tokens("a b c d e f g h"), [blk("a b c X e f g h", 4)], 119, 0)
    assert len(hunks) == 1
    h = hunks[0]
    assert set(h) == {"page", "anchor", "pdf_text", "md_text"}
    assert (h["page"], h["anchor"]) == (119, "p000-b004")
    assert (h["pdf_text"], h["md_text"]) == ("a b c d e f g", "a b c X e f g")


def test_diff_merges_hunks_two_tokens_apart():
    hunks, _ = diff_page(pdf_tokens("a b c d e f g h i j"), [blk("a b X d e Y g h i j", 1)], 1, 0)
    assert len(hunks) == 1
    assert (hunks[0]["pdf_text"], hunks[0]["md_text"]) == ("a b c d e f g h i", "a b X d e Y g h i")


def test_pure_deletion_anchors_to_the_block_matched_just_before_it():
    hunks, _ = diff_page(pdf_tokens("a b c d e f g h"), [blk("a b c", 2), blk("g h", 5)], 1, 0)
    assert len(hunks) == 1 and hunks[0]["anchor"] == "p000-b002"
    assert (hunks[0]["pdf_text"], hunks[0]["md_text"]) == ("d e f", "")
    hunks, _ = diff_page(pdf_tokens("x y z"), [], 1, 3)
    assert hunks[0]["anchor"] == "p003-none"


def test_reordered_boxes_and_footnotes_are_not_hunks():
    blocks = [
        blk("a b c", 0),
        blk("g h i", 1),
        blk("d e f", 2),
        blk("j k l", 3, type="page_footnote"),
    ]
    hunks, aln = diff_page(pdf_tokens("a b c d e f j k l g h i"), blocks, 1, 0)
    assert hunks == [] and len(aln.spans) == 4


def test_run_on_table_fixture_reports_pages_and_counts(table_cache):
    result = run(FIXTURES / "bma" / "table.pdf", table_cache, first_printed=151, stem="table")
    ...
    assert result["counts"]["hunks"] == len(result["hunks"])
    assert all(h["page"] in (151, 152) for h in result["hunks"])


def test_run_rejects_page_count_mismatch(table_cache):
    with pytest.raises(GuardrailError, match="formula.pdf has 1"):
        run(FIXTURES / "bma" / "formula.pdf", table_cache, stem="table")


def test_control_page_has_zero_hunks(text_only_cache):
    result = run(FIXTURES / "bkm" / "text-only.pdf", text_only_cache, stem="text-only")
    assert result["hunks"] == [], json.dumps(result["hunks"], indent=1, ensure_ascii=False)


def test_chart_fixture_figure_captions_are_matched_not_hunked(chart_cache):
    result = run(FIXTURES / "bkm" / "chart.pdf", chart_cache, stem="chart")
    assert not [h for h in result["hunks"] if "Figure 11." in h["pdf_text"]], result["hunks"]
```
