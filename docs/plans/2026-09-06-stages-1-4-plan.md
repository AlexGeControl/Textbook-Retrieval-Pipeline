...
| `src/blocks.py` | `Block` model over `_content_list_v2.json` (Task 7b): spans, per-page ids, type classes |
...
| `metrics/block-types-2026-09-06.md` | block-type evidence from ch05 of all seven books (counts, no book text) |
...
> **Superseded 2026-09-06 by Task 7b** (v2 anchor). The code below is the v1 model as built and
> committed (`9417804`…); it is kept for the record. Do not re-run this task.
...
### Task 7b: Block model over `content_list_v2` (`src/blocks.py`) — the v2 anchor

Added 2026-09-06 after gate 2 (design §1 "Anchor JSON", §6 "Block model"; evidence in
`metrics/block-types-2026-09-06.md`). Replaces the Task 2 model; Tasks 8–10 are written against it.
The code below was dry-run green against the fixture cache in a scratch tree before being pasted.

**Files:**
- Rewrite: `src/blocks.py`, `tests/test_blocks.py`

**Interfaces:**
- Produces: `TEXT_BEARING`, `RUNNING_MATTER`, `VLM_GENERATED`, `FIGURES`, `CAPTIONED`, `MASKED:
  frozenset[str]`; `SCHEMA_TYPE: dict[str, str]`; `class BlockError(Exception)`;
  `block_id(page_idx, index) -> str`; `v2_path(hybrid_auto, stem) -> Path`;
  `@dataclass(frozen=True) Span(type, content)`; `NEWLINE`;
  `@dataclass(frozen=True) Block(id, index, type, page_idx, bbox, spans=(), captions=(), level=None,
  math="", html="", content="", crop="", sub_type=None)` with `text()`, `caption_text()`,
  `inline_math() -> tuple[str, ...]`, `is_vlm_generated() -> bool`;
  `load_pages(path) -> list[list[Block]]`; `load_blocks(path) -> list[Block]`; `on_page(blocks, page_idx)`.
- Gone: `text`, `list_items`, `table_body`, `img_path`, `text_level`, `running_text()`,
  `RUNNING_TEXT`, `DISCARDED`. Nothing else in the repo imports them yet.
...
Run: `uv run pytest tests/test_blocks.py -q`
Expected: FAIL at collection (`cannot import name 'load_pages' from 'src.blocks'`).

- [ ] **Step 3: Rewrite `src/blocks.py`**
...
- [ ] **Step 4: Run — expect 14 passes and a green tier**

Run: `uv run pytest tests/test_blocks.py -v && uv run pytest -q`
Expected: 14 PASSED; the whole unit tier passes.
...
git add src/blocks.py tests/test_blocks.py
git commit -m "Move the block model to content_list_v2: spans, per-page ids, text-bearing classes
...
Revised 2026-09-06 for the v2 anchor: no rule parses markdown. `flatten_latex` is applied span by
span by the guardrail; the string rules run on both sides. Rule 7 came out of the control page
during the dry run (a tracked small-caps running head) and is the first tuning rule on record.

**Files:**
- Create: `src/normalize.py`, `tests/test_normalize.py`

**Interfaces:**
- Produces: `flatten_latex`, `strip_markup`, `nfkc`, `punctuation_variants`, `drop_soft_hyphens`,
  `dehyphenate`, `drop_bullets_and_list_markers`, `join_letter_spaced_caps` — all `str -> str`;
  `tokenize(s) -> list[str]`; `MD_RULES`, `PDF_RULES: tuple[Callable[[str], str], ...]`
  (`MD_RULES[1:] == PDF_RULES`); `normalize(s, rules) -> str`.
...
git commit -m "Add guardrail normalization rules, one test per rule; flatten_latex for v2 spans
...
Revised 2026-09-06 for the v2 anchor: every text-bearing block is aligned (running heads and
folios included), only VLM-body boxes are masked on the PDF side, inline math is flattened span by
span. The original Step 1 mistakenly repeated `test_blocks.py`; these are the guardrail's own tests.

**Files:**
- Create: `src/guardrail.py`, `tests/test_guardrail.py`

**Interfaces:**
- Consumes: `src.blocks` (`CAPTIONED`, `MASKED`, `TEXT_BEARING`, `Block`, `load_pages`, `v2_path`);
  `src.normalize` (`MD_RULES`, `PDF_RULES`, `flatten_latex`, `normalize`, `tokenize`);
  `src.extract` (`STEM`, `hybrid_auto_dir`); `src.config`.
- Produces: `class GuardrailError(Exception)`; `Tok(text, block=None, pos=-1)`;
  `Alignment(residual_pdf, residual_md, spans)`;
  `block_masks(blocks, page_rect, pad=2.0) -> list[pymupdf.Rect]` (MASKED types only);
  `pdf_page_text(page, masks) -> str`; `pdf_tokens(text) -> list[Tok]`;
  `block_text(block) -> str` (body spans for TEXT_BEARING, caption spans for CAPTIONED, inline math
  flattened, spans space-joined); `block_tokens(block) -> list[Tok]`;
  `align_blocks(pdf, blocks) -> Alignment`; `diff_page(pdf, blocks, page_label, page_idx)`;
  `run(chapter_pdf, hybrid_auto, first_printed=None, stem=STEM) -> dict`
...
- [ ] **Step 4: Run — expect 8 passes, control page at zero**

Run: `uv run pytest tests/test_guardrail.py -v`
Expected: 8 PASSED. On the 2026-09-06 cache the control page aligns 48/48 blocks (2495 = 2495
tokens) with zero hunks; the other fixtures keep a handful of hunks that are real mineru defects
(`capi tal`, `ou company`, `NPV=0`), see Task 11 Step 4: table 4 hunks, formula 3 (2 in
inline-math blocks), chart 3.
...
git commit -m "Add stage 3 guardrail: masked PDF text vs content_list_v2 text-bearing blocks, block-anchored hunks

Blocks are matched verbatim first (order-insensitive, so boxed examples,
footnotes, running heads and captions cost nothing), then the residue is
word-diffed. The control page (bkm text-only) aligns 48/48 blocks and reports
zero hunks.
...
Revised 2026-09-06 for the v2 anchor: crops for every VLM body come from mineru (`Block.crop`),
images with `content` are flagged as `figure`, and `page_header`/`page_footer`/`page_number`
blocks whose text is not running matter are flagged as `running_matter_suspect` (design §7).

**Files:**
- Create: `src/qa_schema.py`, `src/qa_report.py`, `tests/test_qa_report.py`

**Interfaces:**
- Consumes: `src.blocks` (`FIGURES`, `RUNNING_MATTER`, `SCHEMA_TYPE`, `TEXT_BEARING`, `Block`,
  `load_blocks`, `v2_path`); `src.normalize` (`join_letter_spaced_caps`); `src.extract` (`STEM`,
  `hybrid_auto_dir`); `src.config`.
- Produces: `class SchemaError(Exception)` with `.path`; `BLOCK_TYPES`, `REASONS`, `STATUSES`;
  `validate(report: dict) -> None`; `BODY_TEXT`; `is_running_matter(text, repeated, chapter_title="")`;
  `running_matter_suspects(blocks, chapter_title="") -> list[Block]`;
  `spot_sample(candidates, book, chapter) -> list[Block]`;
  `render_crop(doc, block, out_dir, zoom=2.0) -> str` (returns `"crops/<id>.png"`);
  `build(meta, guardrail, blocks, middle, content_list_mtime, crop) -> dict`; `main(argv=None) -> int`.
...
git commit -m "Add stage 4 qa_report builder with HANDOVER §6 schema validator; figure and running-matter flags
...
Add `guardrail qa_report chapter` to `.PHONY` and to `help`, and fix the Makefile header comment
(lines 1–2): `make chapter BOOK=<id> CH=<n>` runs stages 1–4. Then:
...
```bash
S=work/gate3  # gitignored scratch for the classification
mkdir -p $S
...
- **inline math** — the owning block has an `equation_inline` span (MFR output);
  `hunks_inline_math` already counts these. They stay: they are review items, not false positives.
...
qa_report counts: ch05 {blocks, tables, formulas, figures, diff_hunks}; ch06 {…}; flagged_blocks by
reason and type for each.
...
```markdown
- Stages 3–5 anchor on `<stem>_content_list_v2.json` (one list per page, span-level text). v1
  and the `.md` are reference renderings; never diff or slice from them.
```
...
Superseded the same day (operator decision): outline quirks are fixed **in the PDF** by
`scripts/patch_toc.py <book> [--apply]` using `toc.patches` rules (`number_from_children`,
`rebuild_numbered_sections`, `insert`, `rename`); `<id>.orig.pdf` keeps the original bytes. The
in-memory `toc.normalize` path was removed; `chapter_entries` is the single title-regex path.

## Addendum 2 — 2026-09-06 (after Task 7 / gate 2): content_list_v2 anchor and block-type evidence

- On operator request `make split extract` also ran on ch05 of acct, bkm, corpfin, ops, stats and
  strat (`metrics/block-types-2026-09-06.md`). Findings: v1 types `aside_text`, `footer`, `ref_text`
  outside the fixture vocabulary; `_middle.json.discarded_blocks` holds five types the `.md` drops;
  the reflowed e-books (acct, ops) get body text labeled header/footer; `image/flowchart` and
  `image/text_image` carry VLM `content`; strat p33 is a blank page → `check_outputs` now counts
  pages from `_middle.json.pdf_info` (`85bc35e`).
- Operator decisions: anchor stages 3–5 on `_content_list_v2.json`; align every text-bearing block
  and mask only VLM bodies; flag images with `content` as `figure`; flag non-running head/foot text
  as `running_matter_suspect`. Design §1/§2/§5/§6/§7/§9/§10 amended; Task 7b added; Tasks 8–10
  rewritten. All four were dry-run green in a scratch tree against the fixture cache (41 tests;
  control page 48/48 blocks, 0 hunks) before being pasted here.
- First tuning rule, from the control page: `join_letter_spaced_caps` (tracked small-caps running
  head `P A R T I I I` vs `PA R T I I I`).

## Addendum 3 — 2026-09-06 (gate 3 closed): handover retired, deferred items

Gate 3 passed on bma ch05/ch06 (`2aba005`; record in `metrics/gate3-bma.md`, eight-chapter evidence
in `metrics/hunk-stats-2026-09-06.md`) and Task 12 followed (`2c2e138`). The session-bridging file
`docs/plans/2026-09-06-gate3-handover.md` (state, order, expected numbers, hunk protocol) is retired
with this addendum; its still-live content is kept here.

**Deferred to the stage-5 plan** (not gate-3 work):

- Stage-5 rendering policy for `page_aside_text` callouts, chart data tables, footnotes and
  `running_matter_suspect` blocks; heading hierarchy must come from `meta.json` (mineru
  `title.level` never goes below 2).
- The stage-5 pre-pass over `diff_hunks`: accept the PDF text when both sides agree after removing
  whitespace and punctuation (keep symbols); inline-math anchors keep their LaTeX and are marked
  verified when the flattened letters match; a chapter-wide move filter (one insertion may pair with
  several deletions); drop list for regions with md words but no text-layer words (buttons, logos,
  icons, drawn bullets). Numbers and the cases where "PDF wins" is wrong are in the hunk-stats file.
- The dominant genuine defect — mineru dropping the last glyph of a right-justified line or a
  trailing footnote superscript (`mineru/utils/span_pre_proc.py` keeps a glyph only if its centre
  is strictly inside the OCR detection span) — deserves a targeted fix or an upstream report before
  whole-book runs.
- acct/ops (reflowed e-books): body text typed page_header/page_footer is flagged as
  `running_matter_suspect` (23 and 3 in ch05) and folios are injected into ops body text; a gate-4
  concern. `chapter_ranges` for stats/acct; the Mac client over Tailscale.

**Engineering notes worth keeping** (from the retired handover §7):

- `ruff check` enforces `re.IGNORECASE` over `re.I` (FURB167), dict literals over `dict(...)`
  (C408) and `\u200b` escapes over raw zero-width spaces (PLE2515); `ruff format --check .`
  covers Python blocks inside Markdown, so plan/design edits must stay ruff-clean.
- PyMuPDF `get_text("words")` keeps em spaces inside a "word", so tests compare word lists, not
  `.split()` counts. Hunk context (`pdf_text`/`md_text`) is drawn from the residual streams after
  block alignment, not from the full page.
- Plan code blocks are extracted with `sed -n` by fence line numbers so invisibles (soft hyphens,
  U+200B) survive byte for byte. The scratch dry-run recipe: a tree that symlinks the repo's
  `src/*.py` (minus the module under test), `tests/helpers.py`, `tests/conftest.py` and
  `tests/fixtures`, with its own `pyproject.toml` (`pythonpath=["."]`), run with
  `uv run --project <repo> --no-sync pytest -q`.
- `MINERU_SERVER_URL` defaults to `http://127.0.0.1:30000` via the Makefile; `extract.py` sets the
  GPU-1 / local-model env itself — never export those by hand. mineru runs ~26 s per bma chapter.
